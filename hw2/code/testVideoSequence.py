...
def updatefig(i):
    ...
    if i in plot_idx:
        plt.savefig("result/tracking_vid" + str(i) + ".png")
    return im,

parser = argparse.ArgumentParser()
parser.add_argument('--num_iters', type=int, default=500, help='number of iterations of Lucas-Kanade')
parser.add_argument('--threshold', type=float, default=1e-2, help='dp threshold of Lucas-Kanade for terminating optimization')
parser.add_argument('--tolerance', type=float, default=0.1, help='binary threshold of intensity difference when computing the mask')
args = parser.parse_args()
num_iters = args.num_iters
threshold = args.threshold
tolerance = args.tolerance

seq = np.load('../data/video.npy')
plot_idx = [10,15,20,25,30]
...
np.save("video.npy", masks)

##### Animation
aerial_masks = np.load("video.npy")
...
for i in range(len(plot_idx)):
    axarr[i].imshow(plt.imread(f"result/tracking_vid" + str(plot_idx[i]) + ".png"))
