...
for i in tqdm(range(1, seq.shape[2])):
    ...
    # p_star = LucasKanade(It0, It1, rect_0, threshold, num_iters)
    # rect = np.concatenate((pt_topleft + p, pt_bottomright + p))

    print("p star ",p_star)
    print("p ",p)
    print("p_accum ",p_accum)

    # if np.linalg.norm(p_star-p_accum) <= threshold_drift:
    if np.linalg.norm(p_star-p_accum) <= threshold_drift:
        # It = seq[:,:,i]
        # p_star = LucasKanade(It0, It, rect, threshold, num_iters)
        # template_rect = np.concatenate((pt_topleft + (p_star-p_accum), pt_bottomright + (p_star-p_accum)))
        # template_rect = np.concatenate((pt_topleft + (p_star-p_accum), pt_bottomright + (p_star-p_accum)))
        rect = np.concatenate((pt_topleft + p, pt_bottomright + p))
    else:
        # rect = np.concatenate(p0_topleft + (p_star-p), p0_bottomright + (p_star-p))
