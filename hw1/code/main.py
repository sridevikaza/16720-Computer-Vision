...
def main():
    ...
    # Q1.3
    img_path = join(opts.data_dir, 'kitchen/sun_afniacozbemafjwf.jpg')
    img = Image.open(img_path)
    img = np.array(img).astype(np.float32)/255
    ...
    # Q2.1
    histog = visual_recog.get_feature_from_wordmap(opts, wordmap)
    hist_plot = plt.bar(x,height=histog)
    plt.show()

    # Q2.2
    spm = visual_recog.get_feature_from_wordmap_SPM(opts, wordmap)
